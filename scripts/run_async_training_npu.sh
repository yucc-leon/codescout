...
# Network proxy for wandb etc.
export http_proxy="http://user:passWorD@192.168.189.225:1080"
export https_proxy="http://user:passWorD@192.168.189.225:1080"
...
CONDA_ENV="${CONDA_ENV:-$CONDA_BASE/envs/codescout-npu-v2}"
...
CKPT_PATH="${CKPT_PATH:-/sharedata/liyuchen/ckpts/code_search_npu/${MODEL_ALIAS}}"
...
$PYTHON -m src.train_npu \
  ...
  generator.inference_engine.http_endpoint_host='0.0.0.0' \
  generator.inference_engine.http_endpoint_port=8080 \
  generator.inference_engine.weight_sync_backend=hccl \
